...
public class LabirintoBuilderTest {
	...
	public void testMonolocaleConAttrezzoSingoloDuplicato() {
		...
		assertTrue(size==1);
		for(Attrezzo attrezzo : monolocale.getStanzaIniziale().getAttrezzi().values()) {
			assertEquals(new Attrezzo("spada",1), attrezzo);
		}
	}
	...
	public void testBilocale() {
		...
		assertTrue(bilocale.getStanzaIniziale().getDirezioni().contains(Direzione.valueOf("nord")));
		assertTrue(bilocale.getStanzaVincente().getDirezioni().contains(Direzione.valueOf("sud")));
	}
	...
	public void testPiuDiQuattroAdiacenti() {
		...
		assertTrue(maze.getStanzaIniziale().getDirezioni2stanze().size()<=4);
		assertTrue(!maze.getStanzaIniziale().getDirezioni2stanze().containsValue(test));
		...
		assertEquals(mappa,maze.getStanzaIniziale().getDirezioni2stanze());
	}
	...
	public void testAggiuntaAttrezziAStanze_AppenaAggiunte() {
		...
		.addAttrezzo(nomeAttrezzo, peso)
		.getLabirinto();
		assertTrue(this.labirintoBuilder.getStanze().get(nomeStanza).getAttrezzi().values().contains(new Attrezzo (nomeAttrezzo,peso)));
		assertEquals(new Attrezzo(nomeAttrezzo,peso),this.labirintoBuilder.getStanze().get(nomeStanza).getAttrezzo(nomeAttrezzo));
	}
	...
	public void testAggiuntaAttrezzoAStanze_AppenaAggiunteMultiple() {
		...
		.getLabirinto();
		Attrezzo attrezzo = new Attrezzo(nomeAttrezzo,peso);
		List<Attrezzo> attrezzi = new ArrayList<Attrezzo>(labirintoBuilder.getStanze().get(nomeStanza).getAttrezzi().values());
	...
	public void testLabirintoConStanzaBloccata_ConPassepartout() {
		...
		assertEquals(stanzaVincente,labirintoBuilder.getStanze().get("stanza bloccata").getStanzaAdiacente("nord"));	
	}
	...
	public void testLabirintoConStanzaBloccata_SenzaPassepartout() {
		...
		assertEquals(stanzaBloccata,labirintoBuilder.getStanze().get("stanza bloccata").getStanzaAdiacente("nord"));
	}
	...
	public void testLabirintoCompletoConTutteLeStanze() {
		...
		assertTrue(corridoio.getDirezioni().containsAll(Arrays.asList(Direzione.valueOf("ovest"), Direzione.valueOf("est"), Direzione.valueOf("nord"), Direzione.valueOf("sud"))));
		Map<Direzione, Stanza> mapAdiacenti = new HashMap<>();
		mapAdiacenti.put(Direzione.valueOf("nord"),new Stanza("corridoio bloccato"));
		mapAdiacenti.put(Direzione.valueOf("sud"),new Stanza(nomeStanzaIniziale));
		mapAdiacenti.put(Direzione.valueOf("est"),new Stanza("stanza magica"));
		mapAdiacenti.put(Direzione.valueOf("ovest"),new Stanza("stanza buia"));
		...
		assertEquals(Arrays.asList(a1,a2), new ArrayList<>(corridoio.getAttrezzi().values()));
	}
}